...
# IICP fallback: derive from index price using stable ratio
# MIDCAPIETF.NS sometimes returns empty on yfinance — ratio is ~0.001036 (very stable)
if iicp_price is None and index_price is not None:
    # Try to compute ratio from last known IICP and index price in history
    known_iicp = None
    known_idx  = None
    for row in reversed(lookback_data):
        if row.get('iicp_price') and row.get('index_price'):
            known_iicp = row['iicp_price']
            known_idx  = row['index_price']
            break
    if known_iicp and known_idx:
        ratio = known_iicp / known_idx
        iicp_price = round(index_price * ratio, 4)
        print(f"  ℹ  IICP derived from index ratio ({ratio:.6f}): {iicp_price}")
    else:
        iicp_price = round(index_price * 0.001036, 4)
        print(f"  ℹ  IICP derived from hardcoded ratio (0.001036): {iicp_price}")
